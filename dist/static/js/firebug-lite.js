var firebug = {
    ...
    el:{}, 
    env:{ 
      "css":"static/js/firebug-lite.css", 
      "debug":true,
      "detectFirebug":true,
      "dIndex":"console", 
      "height":295,
    ...
    init:function(_css){
      ...
      with(firebug){
        ...
        el.button.scripts.lineNumbers = new lib.element("A").attribute.addClass("Button").event.addListener("click",d.scripts.toggleLineNumbers).update("Show Line #").insert(el.button.scripts.container);
    ...
    d: {
      ...
      console:{
        ...
        formatArgs:function(){
          with(firebug){
            var content = [];
            var contentFormatted = false; // CM
            var contentStyle = false; // CM
            for(var i=0, len=arguments.length; i<len; i++){

              if (i === 0 && arguments[i] === '%c%s') {
                contentFormatted = true;
              } else if ( i === 1 && contentFormatted) {
                contentStyle = arguments[i];
              } else {
                content.push( d.highlight(arguments[i],false,false,true,contentStyle) );
              }
            }
            return content.join(" ");
          }
      ...
      highlight:function(_value,_inObject,_inArray,_link,spanStyling){
        with(firebug){
          ...
          try {
            ...
            if(_value==null||vtype=="number"||vtype=="string"||vtype=="boolean"||(vtype=="function"&&_value.nodeName!="OBJECT")||vtype=="regexp"||vtype=="date"){
              if(_value==null){
                ...
                var spanText = ( !_inObject&&!_inArray?_value : _value.substring(0,35)+(_value.length>35?" ...":"") ).replace(/\n/g,"\\n").replace(/\s/g,"&nbsp;").replace(/>/g,"&#62;").replace(/</g,"&#60;");
                spanText = spanText.replace(/\\n/g, '<br />');
                result.push("<span class='console__line__highlight'" + ( spanStyling ? " style=\"" + spanStyling + "\"" : false ) + ">"+spanText +"</span>");
              }
            }
            // element
            else if(isElement){
  ...
  (function(){
    with(firebug){
      ...
      // CM: only ios or if "firebug" is in the url
      const ios = /iphone|ipod|ipad/i.test(navigator.userAgent.toLowerCase());
      const firebugEnable = window.location.search.indexOf('firebug') >= 0;
      if ( ios || firebugEnable ) {
        initConsole();
